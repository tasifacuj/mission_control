...
bool Client::connectPort(const std::string& device, const size_t baudrate) {
    try {
        port.open(device);
        port.set_option(boost::asio::serial_port::baud_rate(uint(baudrate)));
        port.set_option(
...
bool Client::startReadThread() {
    ...
    thread = std::thread([this] {
        boost::asio::async_read_until(port,
                               buffer,
...
bool Client::sendMessage(Message& message, const double& timeout) {
    if(log_level_ >= DEBUG)
...
bool Client::sendMessageNoWait(const Message& message) {
    if(log_level_ >= DEBUG)
    ...
    if(buffer.sgetc() == EOF) {
        if(log_level_ >= WARNING)
            std::cerr << "buffer returned EOF; reading char directly from port"
                      << std::endl;
        boost::asio::read(port, buffer, boost::asio::transfer_exactly(1));
    }
    return uint8_t(buffer.sbumpc());
}

bool Client::sendData(const ID id, const ByteVector& data) {
    if(log_level_ >= DEBUG)
    ...
    std::size_t bytes_written;
    {
        ...
        bytes_written =
            boost::asio::write(port, boost::asio::buffer(msg.data(), msg.size()), ec);
    }
    if(ec == boost::asio::error::operation_aborted && log_level_ >= WARNING) {
...
ByteVector Client::packMessageV1(const ID id,
...
ByteVector Client::packMessageV2(const ID id,
                                 const ByteVector& data) const {
    ByteVector msg;
    msg.push_back('$');                           // preamble1
...
void Client::processOneMessage(const boost::system::error_code& ec,
...
std::pair<BufferIterator, bool> Client::messageReady(BufferIterator begin,
                                               BufferIterator end) const {
    BufferIterator i             = begin;
