...
public:
    ...
    template <typename T, typename C,
              class = typename std::enable_if<
                  std::is_base_of<Message, T>::value>::type>
     ...
     */
    bool hasSubscription(const ID& id) const {
     ...
     */
    std::shared_ptr<SubscriptionBase> getSubscription(const ID& id) {
     ...
     */
    void processOneMessage(const boost::system::error_code& ec,
     ...
     * @return true on success
     */
    bool sendData(const ID id, const ByteVectorUptr&& data) {
...
protected:
     ...
     */
    std::pair<BufferIterator, bool> messageReady(BufferIterator begin, BufferIterator end) const;
     ...
     * @param id ID of the message being packed
     ...
     */
    ByteVector packMessageV1(const ID id,
     ...
     */
    ByteVector packMessageV2(const ID id,
                             const ByteVector& data = ByteVector(0)) const;
...
protected:
    ...
    std::map<ID, std::shared_ptr<SubscriptionBase>> subscriptions;
